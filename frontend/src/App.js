...
const LoginPage = ({ onLogin }) => {
  ...
  const [adminExists, setAdminExists] = useState(true); // default: registration बंद

  // Check करा — पहिला user आहे का नाही
  useEffect(() => {
    axios.get(API + "/auth/check-setup").then(res => {
      setAdminExists(res.data.adminExists);
      if (!res.data.adminExists) setMode("register");
  ...
  return (
    <div style={{ height: "100vh", background: "linear-gradient(135deg, #075E54 0%, #128C7E 50%, #25D366 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Segoe UI', Arial, sans-serif" }}>
      <div style={{ background: "white", borderRadius: 20, padding: 36, width: "min(360px, 90vw)", boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
        ...
        {/* Toggle — फक्त पहिल्या setup वेळी Register दिसतो */}
        {!adminExists && (
...
export default function App() {
  ...
  // Bulk delete + CSV progress states
  const [bulkSelectMode, setBulkSelectMode] = useState(false);
  const [bulkSelected, setBulkSelected] = useState([]);
  const [csvProgress, setCsvProgress] = useState(null); // { current, total, percent, added, skipped }
  ...
  const loadCampaigns = async () => {
    try {
      const [c, s] = await Promise.all([
        ...
        axios.get(API + "/api/marketing/stats")
      ]);
      setCampaigns(c.data);
      setMktStats(s.data);
    } catch {}
  ...
  useEffect(() => {
    ...
    socket.on("csv_progress", (progress) => { setCsvProgress(progress); });
  ...
  const bulkDelete = async () => {
    ...
    if (!window.confirm(`Delete ${bulkSelected.length} contacts? All their messages will also be deleted.`)) return;
    try {
  ...
  const saveTemplate = async () => {
    ...
    try {
      if (editTemplate) {
        await axios.put(API + "/api/marketing/templates/" + editTemplate._id, { name: tplName, body: tplBody, header: tplHeader, footer: tplFooter, category: tplCategory, waTemplateName: tplWaName });
      } else {
        await axios.post(API + "/api/marketing/templates", { name: tplName, body: tplBody, header: tplHeader, footer: tplFooter, category: tplCategory, waTemplateName: tplWaName });
      }
      setShowNewTemplate(false); setEditTemplate(null);
      setTplName(""); setTplBody(""); setTplHeader(""); setTplFooter(""); setTplWaName("");
      loadTemplates();
  ...
  const deleteTemplate = async (id) => {
    ...
    await axios.delete(API + "/api/marketing/templates/" + id);
    loadTemplates();
  };

  const startEditTemplate = (t) => {
    setEditTemplate(t); setTplName(t.name); setTplBody(t.body);
    setTplHeader(t.header || ""); setTplFooter(t.footer || "");
    setTplCategory(t.category); setTplWaName(t.waTemplateName || "");
    setShowNewTemplate(true);
  };

  // 📢 CAMPAIGN functions
  const sendCampaign = async () => {
    if (!campName) return alert("Campaign name required");
    if (!campMessage) return alert("Message required");
    const phones = campLabelFilter === "all" ? contacts.map(c => c.phone) :
      campLabelFilter === "custom" ? campPhones :
      contacts.filter(c => c.label === campLabelFilter).map(c => c.phone);
    if (!phones.length) return alert("No contacts found for this filter");
    setCampStatus("⏳ Starting campaign...");
    try {
      const res = await axios.post(API + "/api/marketing/campaigns", {
        name: campName, message: campMessage,
        phones, labelFilter: campLabelFilter,
        useWaTemplate: campUseWa, waTemplateName: campUseWa ? tplWaName : ""
      });
      setCampStatus(`✅ Campaign started! ${res.data.total} contacts`);
      setCampName(""); setCampMessage(""); setCampLabelFilter("all");
      loadCampaigns();
      setTimeout(() => { setShowNewCampaign(false); setCampStatus(""); }, 3000);
    } catch (err) { setCampStatus("❌ " + (err.response?.data?.error || "Error")); }
  };
  ...
  const Marketing = () => (
    <div style={{ flex: 1, overflowY: "auto", padding: 16, background: T.bg }}>
      <div style={{ fontWeight: "bold", fontSize: 16, marginBottom: 14, color: T.text }}>🎯 WhatsApp Marketing</div>

      {/* Stats */}
      {mktStats && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: 10, marginBottom: 16 }}>
          {[
            { label: "Total Campaigns", value: mktStats.total, icon: "📢", color: "#075E54" },
            { label: "Completed", value: mktStats.completed, icon: "✅", color: "#25D366" },
            { label: "Running", value: mktStats.running, icon: "⏳", color: "#FAB005" },
            { label: "Total Sent", value: mktStats.totalSent, icon: "📤", color: "#3498db" },
            { label: "Failed", value: mktStats.totalFailed, icon: "❌", color: "#e74c3c" },
          ].map((s, i) => (
            <div key={i} style={{ background: T.panel, borderRadius: 12, padding: 12, border: `1px solid ${T.border}`, textAlign: "center" }}>
              <div style={{ fontSize: 20 }}>{s.icon}</div>
              <div style={{ fontSize: 20, fontWeight: "bold", color: s.color }}>{s.value}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 14 }}>
        ...
        <div style={{ background: T.panel, borderRadius: 12, padding: 14, border: `1px solid ${T.border}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <div style={{ fontWeight: "bold", fontSize: 13, color: T.text }}>📋 Message Templates</div>
            <button onClick={() => { setShowNewTemplate(true); setEditTemplate(null); setTplName(""); setTplBody(""); setTplHeader(""); setTplFooter(""); setTplWaName(""); }}
              style={{ padding: "5px 12px", background: "#075E54", color: "white", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 12 }}>➕ New</button>
          </div>

          {showNewTemplate && (
            <div style={{ background: T.bg, borderRadius: 10, padding: 12, marginBottom: 12, border: `1px solid ${T.border}` }}>
              <div style={{ fontWeight: "bold", fontSize: 12, color: T.text, marginBottom: 8 }}>{editTemplate ? "✏️ Edit Template" : "➕ New Template"}</div>
              <input placeholder="Template name" value={tplName} onChange={e => setTplName(e.target.value)}
                style={{ width: "100%", padding: 7, borderRadius: 8, border: `1px solid ${T.border}`, background: T.input, color: T.inputText, fontSize: 12, boxSizing: "border-box", marginBottom: 6 }} />
              <input placeholder="Header (optional)" value={tplHeader} onChange={e => setTplHeader(e.target.value)}
                style={{ width: "100%", padding: 7, borderRadius: 8, border: `1px solid ${T.border}`, background: T.input, color: T.inputText, fontSize: 12, boxSizing: "border-box", marginBottom: 6 }} />
              <textarea placeholder="Message body. Use {{name}}, {{phone}} for variables..." value={tplBody} onChange={e => setTplBody(e.target.value)} rows={4}
                style={{ width: "100%", padding: 7, borderRadius: 8, border: `1px solid ${T.border}`, background: T.input, color: T.inputText, fontSize: 12, resize: "vertical", boxSizing: "border-box", marginBottom: 6 }} />
              <input placeholder="Footer (optional)" value={tplFooter} onChange={e => setTplFooter(e.target.value)}
                style={{ width: "100%", padding: 7, borderRadius: 8, border: `1px solid ${T.border}`, background: T.input, color: T.inputText, fontSize: 12, boxSizing: "border-box", marginBottom: 6 }} />
              <input placeholder="WhatsApp approved template name (optional)" value={tplWaName} onChange={e => setTplWaName(e.target.value)}
                style={{ width: "100%", padding: 7, borderRadius: 8, border: `1px solid ${T.border}`, background: T.input, color: T.inputText, fontSize: 12, boxSizing: "border-box", marginBottom: 6 }} />
              <select value={tplCategory} onChange={e => setTplCategory(e.target.value)}
                style={{ width: "100%", padding: 7, borderRadius: 8, border: `1px solid ${T.border}`, background: T.input, color: T.inputText, fontSize: 12, marginBottom: 8 }}>
                <option value="MARKETING">📢 Marketing</option>
                <option value="UTILITY">🛠️ Utility</option>
                <option value="AUTHENTICATION">🔐 Authentication</option>
              </select>
              <div style={{ display: "flex", gap: 6 }}>
                <button onClick={saveTemplate} style={{ flex: 1, padding: "7px 0", background: "#075E54", color: "white", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 12 }}>💾 Save</button>
                <button onClick={() => { setShowNewTemplate(false); setEditTemplate(null); }} style={{ flex: 1, padding: "7px 0", background: T.input, color: T.text, border: `1px solid ${T.border}`, borderRadius: 8, cursor: "pointer", fontSize: 12 }}>Cancel</button>
              </div>
            </div>
          )}

          <div style={{ overflowY: "auto", maxHeight: 400 }}>
            {templates.length === 0 && <div style={{ color: T.subtext, textAlign: "center", padding: 20, fontSize: 12 }}>No templates yet</div>}
            {templates.map(t => (
              <div key={t._id} style={{ background: T.bg, borderRadius: 10, padding: 10, marginBottom: 8, border: `1px solid ${T.border}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
                  <div>
                    <div style={{ fontWeight: "600", fontSize: 12, color: T.text }}>{t.name}</div>
                    <div style={{ fontSize: 10, color: T.subtext }}>{t.category} {t.waTemplateName ? `· WA: ${t.waTemplateName}` : ""}</div>
                  </div>
                  <div style={{ display: "flex", gap: 4 }}>
                    <button onClick={() => startEditTemplate(t)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 13, color: T.subtext }}>✏️</button>
                    <button onClick={() => deleteTemplate(t._id)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 13, color: "#e74c3c" }}>🗑</button>
                  </div>
                </div>
                {t.header && <div style={{ fontSize: 11, fontWeight: "bold", color: T.text, marginBottom: 2 }}>{t.header}</div>}
                <div style={{ fontSize: 12, color: T.text, lineHeight: 1.4 }}>{t.body}</div>
                {t.footer && <div style={{ fontSize: 10, color: T.subtext, marginTop: 2 }}>{t.footer}</div>}
                <button onClick={() => { setCampMessage(t.body); setCampTemplateId(t._id); setShowNewCampaign(true); }}
                  style={{ marginTop: 8, width: "100%", padding: "5px 0", background: "#25D366", color: "white", border: "none", borderRadius: 6, cursor: "pointer", fontSize: 11 }}>
                  📢 Use in Campaign
                </button>
              </div>
            ))}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ background: T.panel, borderRadius: 12, padding: 14, border: `1px solid ${T.border}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <div style={{ fontWeight: "bold", fontSize: 13, color: T.text }}>📢 New Campaign</div>
              <button onClick={() => setShowNewCampaign(!showNewCampaign)}
                style={{ padding: "4px 10px", background: showNewCampaign ? T.input : "#075E54", color: showNewCampaign ? T.text : "white", border: `1px solid ${T.border}`, borderRadius: 8, cursor: "pointer", fontSize: 12 }}>
            ...
            {showNewCampaign && (
              <div>
                <input placeholder="Campaign name" value={campName} onChange={e => setCampName(e.target.value)}
                  style={{ width: "100%", padding: 8, borderRadius: 8, border: `1px solid ${T.border}`, background: T.input, color: T.inputText, fontSize: 12, boxSizing: "border-box", marginBottom: 8 }} />
                <textarea placeholder="Message... Use {{name}} for contact name" value={campMessage} onChange={e => setCampMessage(e.target.value)} rows={4}
                  style={{ width: "100%", padding: 8, borderRadius: 8, border: `1px solid ${T.border}`, background: T.input, color: T.inputText, fontSize: 12, resize: "vertical", boxSizing: "border-box", marginBottom: 8 }} />
                <div style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 11, color: T.subtext, marginBottom: 4 }}>Send to:</div>
                  <select value={campLabelFilter} onChange={e => setCampLabelFilter(e.target.value)}
                    ...
                    <option value="">📋 Untagged ({contacts.filter(c => !c.label).length})</option>
                  </select>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  <input type="checkbox" checked={campUseWa} onChange={e => setCampUseWa(e.target.checked)} id="useWa" style={{ accentColor: "#075E54" }} />
                  <label htmlFor="useWa" style={{ fontSize: 12, color: T.text, cursor: "pointer" }}>Use WhatsApp Approved Template</label>
                </div>
                {campStatus && <div style={{ fontSize: 12, marginBottom: 8, color: campStatus.includes("✅") ? "#25D366" : campStatus.includes("❌") ? "#e74c3c" : "#FAB005" }}>{campStatus}</div>}
                <button onClick={sendCampaign} style={{ width: "100%", padding: "10px 0", background: "#075E54", color: "white", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: "bold", fontSize: 13 }}>
                  🚀 Launch Campaign
                </button>
              </div>
            )}
          ...
          {/* Campaign History */}
          <div style={{ background: T.panel, borderRadius: 12, padding: 14, border: `1px solid ${T.border}`, flex: 1 }}>
            <div style={{ fontWeight: "bold", fontSize: 13, color: T.text, marginBottom: 10 }}>📊 Campaign History</div>
            <div style={{ overflowY: "auto", maxHeight: 400 }}>
              {campaigns.length === 0 && <div style={{ color: T.subtext, textAlign: "center", padding: 20, fontSize: 12 }}>No campaigns yet</div>}
              {campaigns.map(c => (
                <div key={c._id} style={{ background: T.bg, borderRadius: 10, padding: 10, marginBottom: 8, border: `1px solid ${T.border}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: "600", fontSize: 12, color: T.text }}>{c.name}</div>
                      <div style={{ fontSize: 10, color: T.subtext }}>{new Date(c.createdAt).toLocaleString("en-IN")}</div>
                      <div style={{ marginTop: 6, display: "flex", gap: 6 }}>
                        <span style={{ fontSize: 10, background: "#e8f8f0", color: "#25D366", padding: "2px 6px", borderRadius: 6 }}>✅ {c.sentCount}</span>
                        <span style={{ fontSize: 10, background: "#fff0f0", color: "#e74c3c", padding: "2px 6px", borderRadius: 6 }}>❌ {c.failedCount}</span>
                        <span style={{ fontSize: 10, background: T.input, color: T.subtext, padding: "2px 6px", borderRadius: 6 }}>📊 {c.totalCount} total</span>
                      </div>
                      {/* Progress bar */}
                      <div style={{ marginTop: 6, height: 4, background: T.border, borderRadius: 2, overflow: "hidden" }}>
                        <div style={{ height: "100%", width: `${c.totalCount ? (c.sentCount / c.totalCount) * 100 : 0}%`, background: c.status === "completed" ? "#25D366" : "#FAB005", borderRadius: 2 }} />
                      </div>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                      <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 6, background: c.status === "completed" ? "#e8f8f0" : c.status === "running" ? "#fff8e0" : T.input, color: c.status === "completed" ? "#25D366" : c.status === "running" ? "#FAB005" : T.subtext }}>
                        {c.status === "completed" ? "✅ Done" : c.status === "running" ? "⏳ Running" : c.status}
                      </span>
                      <button onClick={() => axios.delete(API + "/api/marketing/campaigns/" + c._id).then(loadCampaigns)}
                        style={{ background: "none", border: "none", cursor: "pointer", color: "#e74c3c", fontSize: 13 }}>🗑</button>
                    </div>
                  </div>
                </div>
              ))}
  ...
  // ===== MAIN RENDER =====
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "'Segoe UI', Arial, sans-serif", background: T.bg, overflow: "hidden" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", borderBottom: `1px solid ${T.border}` }}>
          {[
            ...
            { id: "marketing", label: "🎯", title: "Marketing" },
          ].map(t => (
            <button key={t.id} onClick={() => { setTab(t.id); if (window.innerWidth <= 768 && t.id !== "chats") setShowSidebar(false); }} title={t.title} style={{
        ...
        {tab === "contacts" && (
          <div style={{ padding: 10, borderBottom: `1px solid ${T.border}`, background: darkMode ? "#1a252e" : "#fafafa" }}>
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 6 }}>
              <label style={{ flex: 1, padding: "6px 0", background: T.input, border: `1px solid ${T.border}`, borderRadius: 8, cursor: "pointer", fontSize: 11, color: T.text, textAlign: "center" }}>
                ...
                <input type="file" accept=".csv" style={{ display: "none" }} onChange={async (e) => {
                  ...
                  try {
                    ...
                    setCsvProgress({ current: res.data.total, total: res.data.total, percent: 100, added: res.data.added, skipped: res.data.skipped, done: true });
                    loadContacts();
                    setTimeout(() => setCsvProgress(null), 4000);
              ...
              <button onClick={() => { setBulkSelectMode(!bulkSelectMode); setBulkSelected([]); }}
                style={{ flex: 1, padding: "6px 0", background: bulkSelectMode ? "#e74c3c" : T.input, color: bulkSelectMode ? "white" : T.text, border: `1px solid ${T.border}`, borderRadius: 8, cursor: "pointer", fontSize: 11 }}>
            ...
            {csvProgress && (
              <div style={{ marginBottom: 6 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: T.subtext, marginBottom: 3 }}>
                  <span>{csvProgress.done ? "✅ Done!" : `Importing... ${csvProgress.current}/${csvProgress.total}`}</span>
                  <span>+{csvProgress.added} added, {csvProgress.skipped} skipped</span>
                </div>
                <div style={{ height: 6, background: T.border, borderRadius: 3, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${csvProgress.percent}%`, background: csvProgress.done ? "#25D366" : "#075E54", borderRadius: 3, transition: "width 0.3s" }} />
                </div>
              </div>
            )}
            ...
            {bulkSelectMode && bulkSelected.length > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 8px", background: "#fff0f0", borderRadius: 8, border: "1px solid #ffcdd2" }}>
                <span style={{ fontSize: 12, color: "#e74c3c" }}>{bulkSelected.length} selected</span>
                <button onClick={bulkDelete} style={{ padding: "4px 12px", background: "#e74c3c", color: "white", border: "none", borderRadius: 6, cursor: "pointer", fontSize: 12, fontWeight: "bold" }}>
          ...
          <div style={{ overflowY: "auto", flex: 1 }}>
            {filtered.map(c => (
              <div key={c.phone}
                onClick={() => {
                  if (bulkSelectMode) {
                    setBulkSelected(prev => prev.includes(c.phone) ? prev.filter(p => p !== c.phone) : [...prev, c.phone]);
                  } else {
                    openChat(c); if (tab !== "chats") setTab("chats");
                  }
                }}
                style={{ padding: "10px 12px", cursor: "pointer",
                  background: bulkSelected.includes(c.phone) ? (darkMode ? "#005c4b" : "#e8f8f0") : active === c.phone ? T.active : T.sidebar,
                  borderBottom: `1px solid ${T.border}`,
                  borderLeft: bulkSelected.includes(c.phone) ? "3px solid #25D366" : active === c.phone ? "3px solid #25D366" : "3px solid transparent" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  {bulkSelectMode && (
                    <input type="checkbox" checked={bulkSelected.includes(c.phone)} readOnly style={{ accentColor: "#25D366", flexShrink: 0 }} />
                  )}
      ...
      {tab === "chats" ? (
          ...
          <div style={{ padding: "10px 16px", background: T.header, color: "white", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            {activeContact ? (
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                {isMobile && (
                  <button onClick={() => setShowSidebar(true)} style={{ background: "rgba(255,255,255,0.2)", border: "none", color: "white", width: 32, height: 32, borderRadius: 8, cursor: "pointer", fontSize: 16, marginRight: 2 }}>←</button>
                )}
        ...
        <div style={{ flex: 1, display: isMobile && showSidebar ? "none" : "flex", flexDirection: "column", overflow: "hidden" }}>
          {isMobile && (
            <div style={{ padding: "8px 12px", background: T.header, display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
              <button onClick={() => setShowSidebar(true)} style={{ background: "rgba(255,255,255,0.2)", border: "none", color: "white", width: 32, height: 32, borderRadius: 8, cursor: "pointer", fontSize: 16 }}>←</button>
              <span style={{ color: "white", fontWeight: "bold", fontSize: 14 }}>
                {tab === "dashboard" ? "📈 Dashboard" : tab === "broadcast" ? "📢 Broadcast" : tab === "reminders" ? "⏰ Reminders" : tab === "pipeline" ? "📊 Pipeline" : tab === "settings" ? "⚙️ Settings" : tab === "marketing" ? "🎯 Marketing" : ""}
              </span>
            </div>
          )}
